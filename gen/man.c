...
static const char *DESCRIPTION =
    ...
    "export, then KEY=...; a commented one is a leading # before the same. Spaces and tabs "
    "around the key and = are accepted, and a UTF-8 byte order mark before an assignment is "
    "ignored. Unquoted values lose outer spaces and tabs; # starts a comment when preceded "
    "by a space or tab. Backslashes stay literal in unquoted values.\n"
    "Single quotes decode \\\\ and \\'. Double quotes decode \\\\, \\\", \\', \\a, \\b, \\f, "
    "\\n, \\r, \\t, and \\v. Backtick quotes decode an escaped backtick or backslash. Unknown "
    "escapes retain their backslash. After a closing quote, only spaces, tabs, and an optional "
    "# comment are allowed. get rejects malformed quoted values with exit 2. Variable "
    "references remain literal; reading values never interpolates the process environment "
    "or evaluates shell expressions.\n"
    "A value that opens a "
...
static const char *REDACTION =
    ...
    "A spelled-out secret word also counts when it ends the key's last segment, because an "
    "English compound carries its head word last, so BW_CLIENTSECRET, GITHUBTOKEN, and "
    "ADMINPASSWORD match, and so do their plurals (DB_PASSWORDS, VAULT_SECRETS, API_KEYS). Only "
    "the last segment counts, so accesstokenExpiry stays visible, and the abbreviations PASS, "
    "PWD, PSK, P12, and a bare KEY still need a segment of their own, which keeps BYPASS_HOSTS, "
    ...
    "with --paranoid.\n"
    "File values are decoded before detection. Unmasked listings and previews retain the "
    "stored spelling, while get returns the decoded contents. A quoted or PEM value spanning "
    "several lines is one logical assignment: masking it "
...
static const char *FILTER_MODE =
    ...
    "Every maskable value in the env file, including disabled assignments, is matched in both "
    "stored and decoded forms, together with its base64, URL-encoded, and JSON-escaped forms. "
    "The value-shape heuristics then run over the rest of "
